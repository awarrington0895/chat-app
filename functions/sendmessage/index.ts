...
import { DynamoDBClient, ScanCommand, ScanCommandInput, ScanCommandOutput } from "@aws-sdk/client-dynamodb";
  ...
  export const handler: Handler = async (event: APIGatewayEvent) => {
    ...
    if (message === '') {
        const errorMessage = 'Body must contain a message attribute with a non-empty string';
        try {
    ...
    try {
        connections = await dynamoClient.send(new ScanCommand({ TableName: process.env.table }));
    } catch (err) {
